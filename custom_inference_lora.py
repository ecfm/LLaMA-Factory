...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('inference_lora.log')
    ]
)
logger = logging.getLogger(__name__)

def parse_args():
    parser = argparse.ArgumentParser(description="Run inference with a LoRA fine-tuned LLM model")
    parser.add_argument(
        "--model_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the LoRA adapter (defaults to model_path if not specified)"
    )
    parser.add_argument(
        "--test_file",
        type=str,
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="inference_results_lora.json",
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Number of items to process in parallel (default: 64 for A100 GPU)"
    )
    parser.add_argument(
        "--save_every",
        type=int,
...
def process_output(output, encoded_input, index, human_message, attention_mask,
    ...
    # Save the result in chat format with metadata
    result = {
        "messages": [
            {
                "role": "user",
...
def extract_question_info(item):
    """Extract question name and title from item if available."""
    # Try to extract question name and title from the item
    question_name = None
    question_title = None
    ...
    if "metadata" in item:
        ...
        if "title" in item["metadata"]:
            question_title = item["metadata"]["title"]
    
    # If not in metadata, check if directly in the item
    if question_name is None and "name" in item:
        question_name = item["name"]
    if question_title is None and "title" in item:
        question_title = item["title"]
        
    return question_name, question_title

def run_inference():
    """Run inference on the test data with the LoRA fine-tuned model using true batching."""
    args = parse_args()
    start_time = time.time()

    logger.info("Optimizing for high-performance single-GPU inference with LoRA models")
    ...
    # If adapter_path is not specified, use the model_path as the adapter path
    adapter_path = args.adapter_path if args.adapter_path is not None else args.model_path
    
    # Setup model with LoRA configuration
    model_args_dict = {
        "model_name_or_path": args.model_path,
        "adapter_name_or_path": adapter_path,  # Path to the LoRA adapter
        "trust_remote_code": True,
        "finetuning_type": "lora",  # Set to LoRA
